function fn_statistics2(page, code, b2b, b2c) {
    ...
    nnf = (document.location.pathname + document.location.hash).replace("/", ""); //현재 페이지 path
    ...
    //var im = new Image();
    //im.onload = function () { };
    //im.src = "/statistics.do?nio=" + nio + "&nbr=" + nbr + "&nsc=" + nsc + "&nbi=" + nbi + "&nla=" + nla + "&nnd=" + nnd + "&nnf=" + nnf + "&nnp=" + nnp + "&nrd=" + nrd + "&nrf=" + nrf + "&nrp=" + nrp + "&b=" + b2b + "&c=" + b2c + "&page=" + page + "&code=" + code + "&timestamp=" + ((new Date()).getTime());

    $.ajax({
        url: ProxyUrl + "/statisticsJson",
        data: {
            "nio": nio,
            "nbr": nbr,
            "nsc": nsc,
        ...
        error: function (e) {
            //$.jQueryAlert("서버와의 통신이 좋지 않습니다. 다시 시도해 주세요!");
        }
    });
...
function fn_statistics(code, b2b, b2c) {
    //alert(code + " " + b2b + " " + b2c);
//	console.log(code + " " + b2b + " " + b2c);
    var nio = '', nbr = '', nsc = '', nbi = '', nla = '', nnd = '', nnf = '', nnp = '', nrd = '', nrf = '', nrp = '';
    var lan = "";
    var ref = "", rh = "", rp = "", rq = "", ru = "", i = "", page = "";
...
function pageCode(v) {
    ...
    if (niIN(v, "SKnowledge/ArticleList/"))  // 상세_간행물(첫단추)
        ...
        page = "D01";
    else if (niIN(v, "magazine/magazineList")) // 매거진 (잡지)_메인페이지
        page = "E01";
    else if (niIN(v, "magazine/popularMagazine")) // 매거진 (잡지)_인기
        page = "E02";
    else if (niIN(v, "magazine/newMagazine")) // 매거진 (잡지)_신간
        page = "E03";
    else if (niIN(v, "magazine/literatureMagazine")) // 매거진 (잡지)_문학
        page = "E04";
    else if (niIN(v, "magazine/artMagazine")) // 매거진 (잡지)_예술
        page = "E05";
    else if (niIN(v, "magazine/newsMagazine")) // 매거진 (잡지)_시사
        page = "E06";
    else if (niIN(v, "magazine/educationMagazine")) // 매거진 (잡지)_교육철학
        page = "E07";
    else if (niIN(v, "magazine/scienceMagazine")) // 매거진 (잡지)_과학기술
        page = "E08";
    else if (niIN(v, "magazine/lawMagazine")) // 매거진 (잡지)_종교법
        page = "E09";
    else if (niIN(v, "magazine/freeMagazine")) // 매거진 (잡지)_무료
        page = "E10";
        ...
        page = "B04";
    else if (niIN(v, "proofreading/info#wordviceModal")) // 영문교정레이어
        page = "F02";
    else if (niIN(v, "proofreading/info")) // 영문교정 안내 페이지
        page = "F01";
    else if (niIN(v, "data-spring/info")) // 영문교정 안내 페이지
            page = "F03";
...
function fn_statisticsFunctionReject(sttCode, code, b2b, b2c, nodeId) {
    ...
    var page = pageCode(document.location.pathname.replace("/", "")); //URL에 따라 공통기능 페이지 코드 변경
    ...
    //var im = new Image();
    //im.onload = function () { };
    //im.src = "/statisticsFunctionReject.do?" + "b=" + b2b + "&c=" + b2c + "&page=" + page + "&code=" + code + "&sttCode=" + sttCode + "&nodeId=" + nodeId;

    $.ajax({
        url: ProxyUrl + "/statisticsFunctionRejectJson",
        data: {"b": b2b, "c": b2c, "page": page, "code": code, "sttCode": sttCode, "nodeId": nodeId},
        type: "POST",
        dataType: "json",
        success: function (result) {
            var data = JSON.parse(result.result);
